...
static const BNO055_CalibProfile_t BNO_OFFSETS_SAVED = {
    .acc_offset_x = -37,
    .acc_offset_y = -16,
    .acc_offset_z = -38,
...
// 2) Boot’ta çağır: Init’ten sonra bu fonksiyonla offsetleri uygula
static void BNO_ApplySavedOffsets(void)
{
  // CONFIG -> offsetleri yaz -> NDOF
  (void)BNO055_WriteCalibProfile(&hi2c1, &BNO_OFFSETS_SAVED);
}
...
void IMU_Boot(void)
{
  ...
  {
    return; // hata yönetimini sen ekle
  }
  BNO_ApplySavedOffsets(); // <<— offsetleri yükle
  // Artık normal okumaya hazırsın (Euler/Quat)
}
